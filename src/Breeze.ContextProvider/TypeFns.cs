...
namespace Breeze.ContextProvider
{
    ...
    public static class TypeFns
    {
      ...
      typeof(Decimal)
        ...
        /// <returns></returns>
        public static MemberInfo FindPropertyOrField(Type type, string memberName, BindingFlags flags)
        {
            foreach (Type t in GetSelfAndBaseTypes(type))
            {
             ...
             MemberInfo[] members = t.GetMembers(flags)
                    .Where(m => 
                         m.Name.ToLowerInvariant() == memberName.ToLowerInvariant() && 
                         (m.GetType() == typeof(PropertyInfo) || m.GetType() == typeof(FieldInfo)))
                    .ToArray();
#else
        ...
        private static MethodInfo[] FindMethodsCore(Type type, string methodName, BindingFlags flags)
        {
#if DNXCORE50
            return type.GetMethods(flags).Where(m => m.Name.ToLowerInvariant() == methodName.ToLowerInvariant()).ToArray();
#else
            return type.FindMembers(MemberTypes.Method,
                flags, Type.FilterNameIgnoreCase, methodName).Cast<MethodInfo>().ToArray();
#endif
        }
        ...
        public static bool IsCompatibleWith(Type source, Type target)
        {
            ...
            TypeCode sc = st.GetTypeInfo().IsEnum ? TypeCode.Object : GetTypeCode(st);
            TypeCode tc = tt.GetTypeInfo().IsEnum ? TypeCode.Object : GetTypeCode(tt);
            switch (sc)
            {
                case TypeCode.SByte:
                    switch (tc)
