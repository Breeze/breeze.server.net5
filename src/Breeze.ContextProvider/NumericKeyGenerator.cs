    ...
    public class NumericKeyGenerator : IKeyGenerator
    {
        ...
        private void AllocateMoreIds(int pCount)
        {
            ...
            try
            {
                ...
                //IDbCommand aCommand = CreateDbCommand(_connection); // WB: original
                DbCommand aCommand = CreateDbCommand(_connection);

                for (int trys = 0; trys <= MaxTrys; trys++)
                {

                    aCommand.CommandText = sqlSelect;
                    //IDataReader aDataReader = aCommand.ExecuteReader(); // WB: original
                    DbDataReader aDataReader = aCommand.ExecuteReader();
...
#if !DNXCORE50
                    aDataReader.Close(); // WB: not implemented by Core
#endif
...
//        private DbCommand CreateDbCommand(IDbConnection connection) // WB: original.
        private DbCommand CreateDbCommand(DbConnection connection)    // WB: DBConnection does NOT inherit from IDbConnection in ASP.NET 5
        {
            // IDbCommand command = connection.CreateCommand(); // WB: original
